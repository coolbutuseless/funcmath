...
double_double_arrow  <-   '%<=>%'      <- function(x, y)  bquote(.(x) %<=>%     .(y))
double_right_arrow   <-   '%=>>%'      <- function(x, y)  bquote(.(x) %=>%      .(y))
double_left_arrow    <-   '%<<=%'      <- function(x, y)  bquote(.(x) %<=%      .(y))
double_up_arrow      <-   '%dblup%'    <- function(x, y)  bquote(.(x) %dblup%   .(y))
double_down_arrow    <-   '%dbldown%'  <- function(x, y)  bquote(.(x) %dbldown% .(y))
